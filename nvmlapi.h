#ifndef _NVMLAPI_H_INCLUDE_
#define _NVMLAPI_H_INCLUDE_
#include <memory>
#include <nvml.h>
#include <json/json.h>
typedef  void* HANDLE;
...
typedef struct _tagGPU_Info
{
	char szName[NVML_DEVICE_NAME_BUFFER_SIZE] = { 0 };
	char szDrvVer[NVML_DEVICE_NAME_BUFFER_SIZE] = { 0 };
	char szUUID[NVML_DEVICE_NAME_BUFFER_SIZE] = { 0 };
	nvmlPciInfo_t nvPciInfo = { 0 };
	nvmlMemory_t  nvMemInfo = { 0 };
	nvmlUtilization_t nvUtilization = { 0 };
	nvmlGridLicensableFeatures_t Licens = { 0 };
	unsigned int nTemperature = 0;
	unsigned int nFanSpeed = 0;
	unsigned int nPowerUsage = 0;
	int nCCCMajor = 0;
	int nCCCMinor = 0;
	int nCudaDrvVer = 0;
	unsigned int mem_clock = 0;
	unsigned int gpu_clock = 0;
	unsigned int video_clock = 0;
	unsigned int sm_clock = 0;
	unsigned int graphics_clock = 0;
	unsigned int max_mem_clock = 0;
	unsigned int max_gpu_clock = 0;
	unsigned int max_video_clock = 0;
	unsigned int max_sm_clock = 0;
	unsigned int max_graphics_clock = 0;
...
{
	NvU32 version;   // 结构体版本
	NvU32 currentLevel;       // 当前级别
	NvU32 minLevel;           // 最低级别
	NvU32 maxLevel;           // 最高级别
	NvU32 defaultLevel;       // 默认级别
...
#define NVAPI_GPU_POWER_TOPO_VERSION MAKE_NVAPI_VERSION(NVAPI_GPU_POWER_TOPO, 1)

#define NVAPI_GPU_POWER_STATUS_VER MAKE_NVAPI_VERSION(NVAPI_GPU_POWER_STATUS, 1)
#define NVAPI_DISPLAY_DVC_INFO_EX_VER MAKE_NVAPI_VERSION(NV_DISPLAY_DVC_INFO_EX, 1)
#define NVAPI_GPU_USAGES_VERSION MAKE_NVAPI_VERSION(NvUsages,1)
#define MAX_GPUS 16
...
public:
	bool Initialize();
public:
	bool UpdateDeviceDetail(std::string& gpu_Detail);
	bool get_uuid(int index, char* uuid, int size);
	bool get_ram_type(NV_RAM_TYPE  nvRamType, std::string& ramTypeString);
	bool get_ram_maker(NV_RAM_MAKER  nvRamMaker, std::string& ramMakerString);
private:
	NvAPI_Status GetUsagesType(NvPhysicalGpuHandle gpuHandle, NvUsages* pnvUsages);
	bool get_hashrate(std::string uniqueId, uint64_t& rate);
public:
...
private:
	static std::auto_ptr<nvmlapi>Instance_;
private:
	HMODULE hNVMLModule_;
	HMODULE hNvidiaModule_;
